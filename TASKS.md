...
| 项 | 值 |
|---|---|
| 分支 | `main` @ `622fe4f`（远端已同步）；本地领先 1 个提交 `2c971f3`（未推送） |
| PR #1 | **已合并**（merge commit `ce506e3`），29 个 conventional commit 进入 main |
| 最新 Release | **20260911**（`app-release.apk` 4.11MB，2026-09-11 08:45Z 发布，说明自动生成） |
...
- [ ] **推送本地 main**（1 个提交 `2c971f3`：`build_apk.py` 版本规则校验）
- [ ] **决定是否重发 Release**：已发布的 `20260911` 仍是 **755 权限逻辑**；main 后续已按用户要求改回 777（`fd334e0`）。若需要发布产物也是 777，需重新触发「发布 Release（纯日期标签）」工作流。

### B. 观察项 / 待确认

- [ ] **主页「shso 目录文件」列表存在显示上限**：实测临时造 12 个文件后列表长度不增加（页面底部内容仍远未触及导航栏）。确认是否为刻意设计；若要完整展开请提出。
- [ ] **「添加到 shso」建目录/拷贝仍是 `chmod 777`**（`RootFileManager` 约 400/407 行）：已确认**刻意保留**（便于其他应用读取），不在改动范围。
...
1. 本地 main 的 `2c971f3` 是否推送。
2. 是否需要重发 Release（当前已发布 `20260911` 含 755 逻辑；main 已改回 777）。
3. 主页「shso 目录文件」列表显示上限是否为刻意设计；是否要完整展开。
