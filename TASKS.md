...
- [x] **文件页搜索 / 过滤**（`a4e7355` 之后一次提交）——已实现
  - 顶栏新增搜索按钮（`content-description="搜索当前目录"`），展开后按名称子串过滤当前目录（大小写不敏感）
  - 过滤与排序**合并为同一次后台遍历**（`applyFileViewSettings(list, showHidden, sortMode, nameQuery)`），大目录不额外多一趟分配
...
- [x] **历史条目来源标记**（`EditHistoryManager.HistorySource` + `data/RelativeTime.kt`）——已实现
  ...
  - **同内容来源升级**：内容相同但来源更强（草稿→快照→保存）时**原地升级来源并保留首次时间**，不再新增重复条目（`HistoryMerge` 纯函数 + 5 例单测）
