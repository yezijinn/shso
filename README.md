...
### v20260904（当前 Release）
- **修复**：无 ROOT 设备文件页显示「当前目录为空」（`pathExists` 增加 Java `File.exists()` 本地兜底，su 失败不再挡住本地回退）。
- **修复**：单顶层文件夹解压产生 `abc/abc` 嵌套（条件 A 解压时 `stripTopFolder` 剥离顶层前缀）。
- **新增**：压缩格式扩展至 14 种（zip/7z/tgz/tar/tar.gz/tar.xz/tar.zst/tar.bz2/tar.lz4/gzip/xz/bzip2/lz4/zstd），移除 RAR。
- **新增**：书签弹窗三段式布局（固定标题 + 可滚动书签列表 + 固定底栏，高度上限 60% 屏高）。
- **优化**：书签弹窗改紧凑视图（纯文本贴边、0 边距 0 行间隔、顶部标题贴顶边、底部按钮贴底边）。
- **优化**：删除「输入要收藏的目录路径」输入框与「收藏当前目录」按钮，仅保留「添加书签」一键收藏当前目录。
