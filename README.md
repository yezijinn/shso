...
- **文本编辑器**：Sora Editor 打开即编辑（4MB/5 万行 ≈0.8s）· 大文件稀疏行索引+虚拟滚动（>32MB 只读）· 外置语法包 62 语言/187 扩展名（按需加载 7–47ms）· 无扩展名按名匹配·顶栏显语言·语法包管理(导入/停用/批量启停)· 原子保存(保留权限属主)· 编码严格报错· 行尾识别 LF/CRLF/CR· 编辑历史来源标记+去重+草稿守卫· 查找/替换(独立游标)· 文本处理二次确认· 写入 BOM· 默认只读双按钮
...
- **终端**：ANSI 解析· 状态指示· 命令历史(50 条/内存不落盘)· 终端设置· 自动跟随· 日志滑动窗口· Toast 统一 LONG· 重启终端代际清理· 守卫探测不阻塞 UI
