...
| 主页 | 文件管理 | 终端 |
| :---: | :---: | :---: |
| ![主页](docs/screenshots/主页.png) | ![文件管理](docs/screenshots/文件.png) | ![终端](docs/screenshots/终端.png) |

| 设置 | 书签 | 文件设置 |
| :---: | :---: | :---: |
| ![设置](docs/screenshots/设置.png) | ![书签](docs/screenshots/书签.png) | ![文件设置](docs/screenshots/文件设置.png) |

| 文本编辑 | 文本对比 | APK 信息 |
| :---: | :---: | :---: |
| ![文本编辑](docs/screenshots/文本编辑设置.png) | ![文本对比](docs/screenshots/文本对比.png) | ![APK 信息](docs/screenshots/APK信息.png) |
...
> 按模块浏览全部能力；详细用法见下方各小节。更新日志见 `更新日志.md`。

- **文本编辑器**：Sora Editor 打开即编辑（4MB/5 万行 ≈0.8s）· 大文件稀疏行索引+虚拟滚动（>32MB 只读）· 外置语法包 62 语言/187 扩展名（按需加载 7–47ms）· 无扩展名按名匹配·顶栏显语言·语法包管理(导入/停用/批量启停)· 原子保存(保留权限属主)· 编码严格报错· 行尾识别 LF/CRLF/CR· 编辑历史来源标记+去重+草稿守卫· 查找/替换(独立游标)· 文本处理二次确认· 写入 BOM
- **文件管理**：目录浏览/记忆/快捷入口· 文件页搜索(名称子串/命中数)· 提取 APK(命名/分包/-splitN/媒体库刷新)· 分包与 XAPK 整体安装· 权限/账户选择器 200+(选应用回填 uid)· 书签预置 /data/adb· 全选文件· 单文件动作(编辑/重命名/拷贝/移动/删除/权限/解压/添加 shso)· 图片浏览· 字体预览· 新建文件· 悬浮导航· 后台长任务前台服务
- **终端**：ANSI 解析(16/256 色·真彩·加粗·退格·行内擦除·OSC 标题与超链接消化)· 长命令运行状态与流式回显· 命令历史(50 条/内存不落盘)· 终端设置· 自动跟随· 日志滑动窗口· 旋转保状态· Toast 统一 LONG· 重启终端代际清理· 守卫探测不阻塞 UI
- **ROOT 安全**：运行时守卫 shso_guard(拦截破坏性写+审计日志)· 安全档位 0–3(自动安装/策略同步/高危确认框/档位0·1红警)· 危险命令拦截(编码混淆/格机原语/重定向检测/变量伪装/脚本来源硬拦/超长拒绝)· 分包安装防护· Zip Slip 防护· 解压目标不可写禁用
- **工程与性能**：安装包体积 2.09MB· R8 keep 收窄· 编译期零警告· 消除 lint Error/告警· 依赖升级· 命名统一· 注释规范化· 文档/命名规范· 历史遗留清理· 编辑历史每文件独立存储
- **其他**：文本对比(逐行/重复)· 压缩包解压 12 种格式(防 Zip Slip)· 图片浏览与字体预览· 添加到 shso(独立存储/自动删除/自动执行)· 执行引擎(RootService/前台服务)· 安全模型(三层防护)· 设置(权限区/行为开关/安全档位/关于)
...
- ANSI 转义序列解析：16 色与加粗，并支持 256 色与真彩色；文字颜色可自定义。
- 控制序列消化：`\r` 原地覆盖、`\b` 退格覆盖、`ESC[K` 行内擦除（进度条只占一行）、
  光标显隐 / 括号粘贴 / 备用屏幕等私有模式序列、OSC 窗口标题与超链接（`ESC ] … BEL|ST`）全部吞掉不留乱码；
  其余 C0 控制字符（NUL / BEL / FF…）不进入文本，复制输出更干净。
- 状态指示：空闲「待命中」，运行中为「运行中」并带循环点动画（每 400ms 递增）。
- 顶栏：`复制输出` / `结束进程` / `重启终端` / `设置`。
- 底部输入行：`中断`（SIGINT）/ `清屏` / `历史` / `Enter`（发送空行）/ `发送`（发送输入框内容）。
- 键入的命令同样登记为「当前任务」：顶栏显示命令名、输出**边跑边显示**（不再等结束才一次性刷出）、
  「中断 / 结束进程」可用且按进程组整组回收；运行超过 5 秒的命令自动进前台保活
  （切后台不被回收，通知内可直接结束）。同一时刻只允许一条命令，重复发送会提示先中断，并保留输入内容。
- 命令历史：列出本次会话执行过的命令（最多 50 条、降序、相邻去重），点击回填输入框，
  支持一键清空；仅存内存不落盘（命令常含密码/token）。
- 终端设置：文字颜色、字号（5–30sp）、`HyperCore 终端提示`、`shso 终端提示`。
- 仅在视图停在底部时自动跟随新日志；日志上限 25 万字符，超出按滑动窗口截断（对齐换行裁剪）。
- 发命令 / 清屏后自动回到日志尾部：上翻读历史时发出的命令，其回显与结果立即可见（不再是「点了没反应」）。
- 单行渲染上限 4000 字符：超长单行（`cat` 二进制 / minified JSON 这类无换行输出）只渲染前 4000 字符
  并标注「本行共 N 字符，已截断显示」；**模型仍是全文，「复制输出」拿到完整内容**。
  （不加此上限时，单行 10 万字符会让 Compose 排版卡住主线程约 20 秒并触发 ANR。）
- 旋转屏幕保留输入框内容、命令历史与尚未确认的高危命令（确认框不会被静默丢弃）。
...
**运行时守卫**（`module/shso_guard`，当前 v1.3.1）
