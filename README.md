...
- 隐藏文件开关、排序（名称 / 时间，升降序）。
- 全选文件 / 取消全选：与「新建文件」同行，只选文件不含文件夹；无文件时给出提示。
- 提取 APK：见下。
...
- 新建文件：设置菜单内，文件名预填当前时间戳，扩展名预填 `txt`。
- 悬浮导航：`⤒` 回顶 / `⤓` 到底 / `⟳` 刷新。
...
- **默认进入只读**，顶栏「只读」「编辑」两个按钮切换并高亮当前状态；只读态下替换、
  历史回退、文本处理等写入入口全部拦截，但**保存不受限**（只读只锁输入，不锁落盘）。
- **任意体积均可编辑**：小于 6.4 万字符走 Compose 文本控件（保留语法高亮），
  超过则切换为原生 `EditText`（增量排版）；超过 128KB 时默认按分段懒加载浏览，
  点「编辑」加载全文后即可改（超过 200KB 会先提示实测耗时）。
- 行号开关（超长文本自动收起）、字号 8–32sp、查找替换（带独立查找游标）、右侧细滚动条。
- 编码：自动检测（BOM → UTF-8 → GB18030 → ISO-8859-1），可手动切换，
  保存时按需写 BOM（面板内可显式开关）。
- 换行：自动识别并在保存时还原 LF / CRLF / CR。
- 编辑历史：每文件最多 20 条、单条 20 万字、单文件总体积 100 万字；仅编辑态产生快照。
