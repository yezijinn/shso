...
- 设置页「检查更新」**Gitee 优先、GitHub 备选**：国内网络访问 GitHub 常超时，
  故以 Gitee 为首选源（走开放 API JSON；其网页版 `/tags` 返回 405），
  GitHub 作备选（走 tags 页面 HTML，正则需忽略大小写）。
  两源归一化规则一致：只取 6..8 位纯数字标签的最大值，与本地 `versionCode` 比较；
  两源均不可达才提示网络异常。
