...
#
# 不变式（INVARIANT）：本文件必须与每个由它生成的守卫「除两处占位符替换外逐字节相同」：
#   1. __CMD_NAME__      → 命令名（如 rm / dd / mv / find / sed / fastboot / wipe）
#   2. __OPERAND_MODE__  → 操作数提取模式（ARGS / DD / FASTBOOT / MVCP / FIND / SED）
# 决策/审计/防递归逻辑只在 guard/common.sh 的 run_guard()，此处不得复制决策代码；
# 改动公共行为请改 common.sh 后「重新生成所有守卫」。
# 注：toybox / busybox 守卫结构不同（多二进制派发），不由此模板生成，见 guard/toybox、guard/busybox。
#
# 安装方式无关：既支持刷 zip（customize.sh），也支持直接拷到
# /data/adb/modules/shso_guard（App 复制安装）——本文件须开箱即用，不依赖安装期产物。
#
# 拦截流程：
