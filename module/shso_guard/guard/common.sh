...
# 兼容性：以 POSIX 为主、不依赖 bash 专有设施；唯一例外是 sanitize_field 用于匹配换行的
# ANSI-C quoting（美元符 + 单引号包裹的反斜杠 n）—— mksh、busybox ash 都支持，dash 不支持，
# 故本模块适用面为 mksh / busybox ash，不含 dash。
# 注意：注释里不得直接书写该字面量，写错会闭合引号/断行，直接破坏整个文件的解析。
#
# 设计要点
#   1. 策略文件缺失时使用内置兜底清单，保证「裸装也受保护」。
#   2. allow 先于 protect 判定，故 /data/media 可豁免 /data 的保护。
#   3. 路径判定前必须归一化，否则 /system/../data 可绕过。
#   4. 匹配 protect="/" 时只判精确相等，否则 `/*` 会误伤全部路径。
#   5. 决策逻辑集中在 run_guard()，所有守卫（含 toybox/busybox 派发）共用，禁止复制决策代码。
