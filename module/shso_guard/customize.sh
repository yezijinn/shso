...
#   B. 复制安装：整目录拷到 /data/adb/modules/<模块ID>，本脚本【不】执行，
#      故模块须开箱可用：守卫已预生成、policy.conf 随模块自带（service.sh 开机兜底部署）。
#
# 约束（模块铁律）：
...
# 本模块不使用 system/ 目录（纯脚本 + 运行期 PATH 拦截），不依赖 KernelSU 的
# metamodule，三种方案零额外依赖。
