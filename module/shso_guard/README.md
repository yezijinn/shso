...
守卫拦的对象就是「接下来要执行的命令」，如果策略与审计路径能由环境变量随便指定，
一条命令就能把防护关掉。现在两个路径变量只认受信目录前缀，轮转阈值与深度计数也不再可覆盖。

| 变量 | 行为 |
|---|---|
| `SHSO_POLICY` | 只接受 `/data/adb/shso_guard/` 下的路径（该目录 root 0755），含 `..` 的回退路径一并拒绝，其余回落默认策略文件。收紧前实测 `SHSO_POLICY=<mode=off 文件> rm -rf /system/x` 可直接放行 |
| `SHSO_AUDIT` | 只接受 `/data/adb/shso/` 下的路径，规则同上 |
| `SHSO_AUDIT_MAX` | 不再生效，固定 2000 行。它只影响日志长度，留作覆盖只会被用来撑分区 |
| `SHSO_GUARD_DEPTH` | 辅助计数，须为十进制，上限 3 |

需要临时放宽拦截时，改策略文件里的 `mode=log`。
...
日志超过 `AUDIT_MAX_LINES + 400` 行时，保留尾部 2000 行（v1.3.0 起不再接受
`SHSO_AUDIT_MAX` 覆盖）。轮转用每进程唯一的临时名 `audit.log.$$.tmp`，避免并发调用互相覆盖；
行数统计按 PID 抽样，每 16 次调用查一次，把 `wc -l` 的整文件扫描成本摊薄。

落盘前有两道处理（v1.3.0 起）：

1. 字段清洗：字段内的 `|` 转义为 `\u007C`，换行转义为 `\n`，控制字符剥离。审计行以 `|`
   分隔字段，不清洗时字段会错位，换行还能注入一整行伪造记录。
2. 目标校验：`audit.log` 若是软链或非常规文件，先用 `/system/bin/rm -f --` 清除，清不掉
   就放弃这次写入，不会退化成继续追加。审计目录是 0777（刻意留的，方便第三方文件管理器访问），
   第三方能在里面放软链让 `>>` 跟着写到任意文件，所以这一步不能省。
